...
const checkUserEmail = async (req, res) => {
  ...
  try {
    ...
    crypto.randomBytes(32, (err, buff) => {
      ...
      pool.query(
        "UPDATE users SET user_reset_token = $1 WHERE user_email = $2",
        [token, email]
      );
      ...
      transporter.sendMail(mailOptions, (error, info) => {
        ...
        return res
          .status(200)
          .json({ message: "User has an account and email sent" });
      });
    });
  } catch (err) {
...
const validateToken = async (req, res) => {
  ...
  try {
    const user = await pool.query(
      "SELECT * FROM users WHERE user_reset_token = $1",
      [token]
    );
    if (user.rows.length === 0) {
...
const setNewPassword = async (req, res) => {
  ...
  try {
    ...
    try {
      await pool.query(
        "UPDATE users SET user_password = $1 WHERE user_reset_token = $2",
        [bcryptPassword, urlToken]
      );
      // remove token from database upon successful update
      await pool.query(
        "UPDATE users SET user_password = $1, user_reset_token = NULL WHERE user_reset_token = $2",
        [bcryptPassword, urlToken]
      );
    } catch (err) {
      return res.status(500).send("Server Error");
